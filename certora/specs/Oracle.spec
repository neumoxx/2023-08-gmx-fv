...
rule minAlwaysLTEMax(method f) filtered {
    f -> (
      notHarnessCall(f) &&
      // I filter out setPrimaryPrice because it does not check for consistency in min/max,
      // because it's used only in simulations
      f.selector != sig:setPrimaryPrice(address, Price.Props).selector &&
      // I filter setPrices and validatePrices because I cannot avoid the timeout
      f.selector != sig:setPrices(address,address,OracleUtils.SetPricesParams).selector &&
      f.selector != sig:validatePrices(address,OracleUtils.SetPricesParams).selector
    )
} {
